...
export function showNewsModal(news) {
  ...
  modal.querySelector("#news-ok").addEventListener("click", close);
}
  ...
  el.innerHTML = `
    <div class="topbar">
      ...
      <div class="title"><div class="t">${iconBell()} Powiadomienia</div>
        <div class="topic">${state.notifUnread ? `${state.notifUnread} nowych` : "wszystko odhaczone"}</div></div>
      ${("Notification" in window) && Notification.permission !== "granted"
        ? `<button class="pillbtn" id="btn-notif-perm">Włącz powiadomienia systemowe</button>` : ""}
      <button class="btn ghost" id="btn-notif-read" ${state.notifUnread ? "" : "disabled"}>Odhacz wszystkie</button>
    </div>
    <div class="notif-list viewfade" id="notif-list">
      ${list.length
        ...
        <button class="notif ${n.readAt ? "" : "fresh"}" data-notif="${n.id}">
          ...
          <span class="nb">
            <span class="nh"><b>@${escapeHtml(n.from ?? "?")}</b> ${NOTIF_OPIS[n.kind] || "zrobił(a) coś"}
        ...
        : emptyStateHtml(iconBell(2.4), "Nic nowego", "Tu trafiają zawołania, wiadomości prywatne, reakcje i zmiany Twoich stron wiki.")}
    </div>`;
